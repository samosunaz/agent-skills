...
- **Interrogate before done**: the agent runs `/samuel:interrogate` on its **own** work at every close-out — `implement` Step 3, `validate` Step 2, `iaas` Simplify, `coordinate` C6 — and prints its one-line verdict in that skill's report. It is the author's question (should this exist?), asked before the reviewer's (is it correct?); a human never has to type it, and "nothing to cut" is a valid answer that costs one line.
