...
Codex's optional companion files are `.mcp.json` (`mcpServers`) and `.app.json` (`apps`) — this repo declares neither. `hooks` **is** accepted: `.codex-plugin/plugin.json` may name a hooks file by path, and `shunt` declares one (verified against Codex CLI 0.154.0 — `codex plugin add` installs the manifest without complaint). That version reads the field and runs no handler from it, so a plugin's gate is installed per repo until that changes (`plugins/shunt/scripts/install-codex.sh`). There is no OpenAPI document anywhere in the plugin contract; the only YAML is a per-skill `<skill>/agents/openai.yaml` carrying Codex-side presentation metadata (`interface.display_name`, `interface.short_description`, optional `icon_small`/`icon_large`/`brand_color`/`default_prompt`, `policy.allow_implicit_invocation`, `dependencies.tools`). It is optional and unused here; the `agents/` directory at plugin root is Claude sub-agents, unrelated.
...
- **`/samuel:team-orchestrate`** — Spawn multi-session Claude Code agent teams (review/debug/feature/research archetypes). Use for parallel work streams that need a **shared task list** (`blockedBy`), structured protocol messages, and a lead-controlled lifecycle. Different from `/samuel:codebase-documentation`, which is a one-shot subagent fan-out — and from plain cross-session messaging, which since 2.1.224 lets sessions you started yourself talk with no flag, no tmux, and no team (`reference/cross-session.md`); "my sessions should talk" is no longer a reason to spawn a team.
- **`/samuel:coordinate`** — **Single-task coordinator over Orca**: the session owns direction, decomposition and sign-off and hands every routine implementation to a briefed worker (Claude `opus` for frontend/UI/copy, Codex for backend/architecture/debugging/tests, a *different* model for independent review). Every worker is launched with an explicit model and effort through `orca orchestration worker-start` and the receipt's `launch.effective` is checked — a mismatch stops the run rather than continuing on inherited defaults. Names are `<task>-<role>-<model>`, passed as `--name` so Orca derives branch, path and card from it (no renames); writers get their own worktree, readers share the current one. Standing constraints live in `.claude/run-policy/{run_id}.md` (keyed by the Orca Run, never a checkout-level path — sibling sessions in one checkout overwrite those) and are appended to every brief, so the human states them once. The brief has six slots (deliver · start commit · may change · must not touch · verify · artifact) and the worker returns six lines (result · frozen commit · files · tests · screenshots · unresolved); the coordinator reads the report first and opens the diff only on scope/security/behaviour risk. Supervision is rolling nine-minute `check --wait` windows with one status line each, a proven start before any window opens, and never ending the turn with a dispatch open. Integration and the project's real checks happen in one checkout; push/PR/publish need explicit approval. Boundary: `waves` runs a planned issue set, `conductor` runs the pipeline for one issue, `team-orchestrate` runs conversing Claude peers — `coordinate` takes one task, planned or not, and its workers always talk Orca mail (an issue's Executor Plan becomes the brief); running `conductor` or `iaas` inside a worker is opt-in (`--via conductor`, `--via iaas`), never the default — `--via iaas` trades the briefed reviewer for blind audit rounds over a draft PR. Recipes: `plugins/samuel/skills/coordinate/references/dispatch-protocol.md`; brief + report contract: `references/worker-brief.md`; what Orca itself provides (agent hooks → `worker-read` transcripts and `worker-show` `agentWait`, managed accounts for quota deaths, cards as the kanban, `file open-changed` for in-editor review, embedded-browser captures, automations with `--precheck` as the nocturnal trigger, paired environments for remote workers): `plugins/samuel/reference/orca-substrate.md` — shared with `waves` and `team-orchestrate`.
...
`feature_dir`: `docs/features/<slug>/`. Always derive it from the item's task-context file, never re-type it.
...
- **Every posted artifact carries run metadata**: any comment, review, or PR body a pipeline phase posts appends a `samuel:run` HTML-comment block (`phase`/`agent`/`model`/`effort`/`sha`/`round`) as its own separate block, never merged into an existing pass marker — `plugins/samuel/reference/github-operations.md` § Run metadata.
...
- **Interrogate before done**: the agent runs `/samuel:interrogate` on its **own** work at every close-out — `implement` Step 3, `validate` Step 2, `iaas` Simplify (first of its three passes, before native `/simplify` and `remove-slop`), `coordinate` C6 — and prints its one-line verdict in that skill's report. It is the author's question (should this exist?), asked before the reviewer's (is it correct?); a human never has to type it, and "nothing to cut" is a valid answer that costs one line.
...
**NO shell expansion in inline `!` commands.** The permission checker rejects any inline command containing command substitution `$(...)` / backticks, parameter expansion `${v:-X}`, or variables `$VAR` — it fails with `Contains expansion` and the command never runs (a fallback does NOT save it). A subshell `( )` fails one step earlier: the engine never decomposes it into the commands inside, so no allowlist rule can ever match one, whatever it declares — write the logic flat (`a || b`, never `(a) || (b)`). To read `.claude/task-context/{item}.md` frontmatter, use the **canonical resolver** documented in `reference/task-context.md` § Reading — one `awk -v k=<key> -v d=<sentinel> '…'` call whose program is byte-identical in every skill (`scripts/lint-skill-context.sh` rule 4 rejects a drifted copy); it picks the item's file from the branch number inside awk, so the shell never sees a `$`. Compound checks that need a value interpolated into a path (e.g. "does `features/<slug>/spec.md` exist?") cannot be expressed `$`-free — do those in the skill body with `Read`/`Glob`, not in `## Context`.

**Every binary a command runs MUST appear in that skill's own `allowed-tools`.** The frontmatter is resolved before the skill runs, so an undeclared binary is denied on any repo whose settings do not happen to cover it — and headless (`claude -p`) that denial is silent: `is_error: false`, exit 0, `num_turns: 0`, empty output. An unattended `/samuel:conductor` or `/samuel:waves` run looks successful having done nothing. Declare every binary in the pipeline, not just the first: `git status … | wc -l | xargs` needs `Bash(wc *)` **and** `Bash(xargs *)`. `scripts/lint-skill-context.sh` enforces all three rules — including over `template/SKILL.md`, the file every new skill is copied from.
