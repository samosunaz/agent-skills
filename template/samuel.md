...
- **`signoff`** — optional, and **shipped commented out on purpose** for the same reason as `autonomy`: it writes a commit status to GitHub, and a repo that copied this file verbatim would start asserting a green check nobody configured. Uncomment it when the repo has deliberately traded a CI job for the local gate — `gh signoff gate` ([basecamp/gh-signoff](https://github.com/basecamp/gh-signoff)) creates the `signoff/gate` status that branch protection then requires instead of the CI job. `/samuel:done` runs it in Step 3 **after the push**: the extension refuses a commit that isn't on a remote.
  - **It signs only what actually ran.** Name the contexts the local gate covers and nothing else. A job that still runs remotely — a browser e2e suite, a cross-platform matrix — keeps its own required check; signing that one locally swaps a real verification for an assertion about it, and the swap is invisible on the PR because both render as a green check.
  - `/samuel:done` refuses to sign when `HEAD` moved past the SHA `/samuel:validate` gated: the status would attest a commit no gate ever saw.
  - Same `#` restriction as `security_scan` (the reader strips from the first `#`), and the same allowlist requirement for an **autonomous** run.
- **`autonomy`** — optional, and **shipped commented out on purpose**: a repo that copies this file verbatim must get today's behaviour, not ten checkpoints silently switched off by a template it never read. Uncomment to opt in. How a **soft** checkpoint behaves: `interactive` asks and waits (the default, and what you get by deleting the line); `attended-auto` takes the obvious default and announces it in one line instead. Hard stops — plan-reality mismatch, constitution violation, red gate, incomplete DoD, any outward action — keep waiting at every level. Gate-by-gate table and the recording contract: `plugins/samuel/reference/autonomy.md`.
