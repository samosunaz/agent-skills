...
- Under Claude Code auto mode the classifier can deny the injected `awk` read of the key even though the skill declares `Bash(awk *)`, and the skill aborts before its body loads. That is a permission gap, not a flaky command: do not retry it, and do not edit `permissions.allow` yourself (writes to the agent's own allowlist are denied too). Hand the user the one-liner and continue once it ran: `jq '.permissions.allow += ["Bash(awk *)"]' ~/.claude/settings.json > /tmp/settings.json && mv /tmp/settings.json ~/.claude/settings.json`. An explicit allow rule bypasses the classifier.
