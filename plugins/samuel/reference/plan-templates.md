...
**Direct lane.** An **S** with no behaviour change — a stylesheet, a token, copy, a doc — whose result the human judges from a capture or a one-minute diff read skips the machinery: TL;DR on the item, then implement in the session that is open, gate, PR. No Executor Plan, no worker, no audit loop; when the direction is open, one prototype round at most (a few variants, one capture each), and the pick is lifted in the same session. The TL;DR says so with a `direct` chip after the size (`reference/github-operations.md` § TL;DR); `/samuel:iaas` and `/samuel:waves` read that chip and stand down. The chip is set at capture or plan time and read afterwards, never re-judged mid-run — a thirty-line change wrapped in fresh-context phases is how an S costs an hour.
