...
- **Slug**: kebab-case, derived from the capability name (`scheduled-publishing`). A slug in the dossier's `--lang` language is fine too — pick one language per catalog and stay consistent. Reuse `feature_slug` from `.claude/task-context/{item}.md` when feature-aware.
