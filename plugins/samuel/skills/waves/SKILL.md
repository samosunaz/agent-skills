...
allowed-tools: Bash(orca *) Bash(gh *) Bash(git fetch *) Bash(git branch *) Bash(git status *) Bash(jq *) Bash(sed *) Bash(tail *) Bash(head *) Bash(awk *) Bash(grep *) Bash(xargs *) Bash(test *) Bash(date *) Bash(cat *) Bash(ls *) Read Edit Write Skill AskUserQuestion PushNotification ListAgents SendMessage
---
...
**Boundary.** Different from `/samuel:conductor` — the per-item engine waves dispatches (as the claude-worker variant) and never reimplements — and from `/samuel:team-orchestrate` — Claude peers that converse; wave workers are isolated implementers reporting lifecycle only. Anti-double-scheduler: an Orca automation may *invoke* waves on a schedule; waves never duplicates conductor's pipeline logic and never schedules itself. This skill is **attended-only** — a live coordinator session with the human reachable; the unattended/nocturnal variant needs a run TTL and an automatic report before it can exist, and neither is built yet.
...
- A `claude -p` worker launched without `--settings "{\"crossSessionInbound\":\"accept\"}"` **holds every inbound message forever** — a `-p` session cannot render the approval dialog, and the coordinator sees no error, only silence. The JSON needs escaped double quotes because Orca's `--command` is already single-quoted.
- A worker started in **bare mode** binds no inbox socket: invisible to `ListAgents`, unreachable, no error anywhere. Never launch a wave worker that way.
- Codex workers are not on the peer roster at all — cross-session messaging is Claude-to-Claude. Nothing in P4/P5 that messages workers applies to the default engine.
...
- A headless `claude -p` worker **exits the moment it ends a turn while a Bash `run_in_background` task is still running** — the task is killed (`task_updated … killed` in the JSONL) and the run ends `subtype: success` with uncommitted work and no PR. The conductor pipeline reaches for `run_in_background` on the gate by itself. Put "run the gate and every long command in the FOREGROUND with the maximum Bash timeout; never run_in_background" in the claude-variant goal, and treat a `success` result whose text reads "waiting on the gate" as an aborted item: relaunch in the same worktree, the conductor resumes from `task-context.md`.
- `worker-show --dispatch <id>` now reports `observation.agentWait` (Orca ≥ 1.4.193, needs `orca agent hooks on`): the structured form of the lost-Enter and update-dialog checks above. `prompt-text` = nudge with one Enter; a permission prompt in a claude worker = the allowlist was wrong, relaunch; **absent** = Orca never looked, read the terminal. Substrate facts: `../../reference/orca-substrate.md`.
- The two-step Codex launch (custom argv) needs `orca agent hooks prepare-codex` first, or the worker's hooks are untrusted and `worker-read` falls back to a terminal tail for the whole run.
