...
9. **CLOSE THE RUN** — sort every finding the run surfaced into FOLD / FILE / DROP (`../../reference/finding-verdicts.md`): FOLDs found after merge go to one closing fold PR through the same tiers (never to `/samuel:polish`, which freezes behaviour and reverts new helpers); FILEs become one issue per area, DROPs stay one line in the ledger; report the net delta (closed − filed). Then run `/samuel:land --dry-run` and print what it returns: the train this run's PRs would join and the branches, worktrees and merged leftovers it would sweep. It merges and deletes nothing; it exists so the run's exit state is a list the human can approve later, not something to rediscover by hand. A run that ends at a verdict leaves its PR, branch and worktree behind — seven open PRs and twelve worktrees accumulated in one week that way.
...
- One unattended run closed 16 items and surfaced 18 follow-ups, none of them a regression it introduced: planners cut three-line fixes in touched files to keep items at S, and every side note became an issue. Findings carry a FOLD / FILE / DROP verdict from the moment they are noticed (`../../reference/finding-verdicts.md`), and the run reports its net delta.
- The stash list is shared by every worktree of a repo. An implementer's `git stash push` failed on a bad pathspec, and the `git stash pop` after it applied another session's `stash@{0}` into its worktree (79 foreign paths). Briefs and reviewer contracts forbid `git stash`; a red-on-main proof restores files from copies or `git show <base>:<path>`.
