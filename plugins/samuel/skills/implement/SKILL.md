...
2. **Set phase**: edit `.claude/task-context/{item}.md` → `phase: implement`, `last_updated: {today}`.
...
**f. Commit the step** (when the run has commit authority — interactive user-approved, `attended-auto`, or autonomous; a local commit is not an outward action). `../create-atomic-commit` is cited for the message convention, not dispatched as a gate — commit inline here with `git add`/`git commit`, so that skill's own confirmation is never rendered from this path: an atomic conventional commit (`../create-atomic-commit`). Include the journal file. Never push / open a PR here.
