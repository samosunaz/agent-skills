...
- The scan command is repo-defined, so it won't match this skill's `allowed-tools` patterns beyond `gitleaks`/`semgrep`. Interactive runs prompt; an **autonomous** run needs the command in the conductor's allowlist (`../../workflow/conductor/references/autonomous-run.md` § Permission allowlist, and the pre-staged block in `assets/conductor.yml`) or the scan dies unapproved and the run reads as skipped — a `SKIP` weighs nothing, so the gate passes without scanning.
