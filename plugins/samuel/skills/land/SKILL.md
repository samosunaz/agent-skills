...
2. **CLASSIFY** — every candidate becomes exactly one of: **ready**; **needs-update** (behind base, or a green check/signature that belongs to an older SHA); **blocked** (a red or missing required check, an unresolved Blocker in review, a draft with no review evidence). A blocked PR leaves the train carrying its reason and the skill that owns the fix. Nothing is repaired here. A **draft** with no review evidence and no commit in the last three days is additionally marked **stale**, with its age and its two exits: it joins this train after an update, or it is closed with its branch kept (`gh pr close` deletes nothing) and its issue returned to `pipeline:planned` with the reason. The human picks per PR; a stale draft is never carried silently into the next dry-run.
...
- **Runs end at a verdict, and nobody runs land.** Seven open PRs and twelve worktrees accumulated in one week because `/samuel:cascade` and `/samuel:iaas` stopped at the draft PR. Both now close with `--dry-run`; a session that produced a PR and did not print that list has not finished.
- **A branch is not an orphan because one checkout's `origin/<branch>` ref is missing.** A sibling worktree may have pushed it; read the branch through the host API before reporting unpushed work, and before pushing "as a backup".
